...
workflow BAM_QC {
    ...
    ch_qualimap_bed // path (single prepared BED, value channel)
    ch_ref          // val [meta_ref, fasta]
    ch_vbid_svd     // val [ud, mu, bed]
    ch_vbid_refvcf  // val file
    ...
    // Derive [meta, bam] from ch_bam_bai — avoids double-consuming ch_bam_final
    def ch_bam = ch_bam_bai.map { meta, bam, bai -> tuple(meta, bam) }
